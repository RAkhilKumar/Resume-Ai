...
interface AuthCtx { user:User|null; session:Session|null; loading:boolean; signUp:(e:string,p:string,n:string)=>Promise<{error:Error|null}>; signIn:(e:string,p:string)=>Promise<{error:Error|null}>; signOut:()=>Promise<void> }
...
export function AuthProvider({ children }:{ children:ReactNode }) {
  ...
  useEffect(()=>{
    ...
    const {data:{subscription}} = supabase.auth.onAuthStateChange((_,session)=>{ setSession(session); setUser(session?.user??null); setLoading(false) })
